...
function Title({main, title})
{
    ...
    return (
        <Tag className={classNames(classes.title, main_class)}>{title}</Tag>
    )
}
...
function Description({main, description})
{
    ...
    if (Array.isArray(description))
    {
        ...
        return (
            <h3 className={classNames(classes.description, main_class)} dangerouslySetInnerHTML={{ __html: description_tags }}></h3>
        )
    }

    return (
        <h3 className={classNames(classes.description, main_class)}>{parse(description)}</h3>
    )
}
